/**
 * js-library-detector 库的检查文件
 * 源码https://github.com/johnmichel/Library-Detector-for-Chrome/blob/master/library/libraries.js
 */
...
var d41d8cd98f00b204e9800998ecf8427e_LibraryDetectorTests = {
  ...
  Dojo: {
    ...
    test: function (win) {
      if (win.dojo && win.dojo.delegate) {
        var version = win.dojo.version
          ? win.dojo.version.toString()
          : UNKNOWN_VERSION;
        return {
          version: version,
  ...
  Underscore: {
    ...
    test: function (win) {
      if (
        ...
        !d41d8cd98f00b204e9800998ecf8427e_LibraryDetectorTests["Lo-Dash"].test(
          win
        )
      ) {
  ...
  "Create React App": {
    ...
    test: function (win) {
      ...
      if (
        root &&
        noscript &&
        /You need to enable JavaScript to run this app/.test(
  ...
  "October CMS": {
    ...
    npm: null,
    test: function (win) {
      const generatorMeta1 = document.querySelector(
        'meta[name="generator"][content^="OctoberCMS"]'
      );
      const generatorMeta2 = document.querySelector(
      ...
      if (
        generatorMeta1 ||
        generatorMeta2 ||
        res.some((s) => resourcesOctober.test(s.src || s.href))
      ) {
  ...
  Joomla: {
    ...
    test: function (win) {
      ...
      const hasJoomlaBootstrap = !!document.querySelectorAll(
        'script[src*="/media/jui/js/bootstrap.min.js"]'
      ).length;
